...
namespace MoreMountains.TopDownEngine
{
	...
	[AddComponentMenu("TopDown Engine/Character/Abilities/Character Orientation 25D")]
	public class CharacterOrientation25D : CharacterAbility
	{
		...
		/// if this is false, the model won't visually rotate but direction will still be tracked for animator
		[Tooltip("if this is false, the model won't visually rotate but direction will still be tracked for animator")]
		public bool ApplyVisualRotation = true;
		...
		public float MovementDirectionX { get { return _movementDirectionX; } }
		public float MovementDirectionZ { get { return _movementDirectionZ; } }
		...
		protected float _movementDirectionX;
		protected float _movementDirectionZ;
		...
		protected int _movementDirectionXAnimationParameter;
		protected int _movementDirectionZAnimationParameter;
		...
		protected override void Initialization()
		{
			...
			{
				Debug.LogError("CharacterOrientation25D on "+this.name+" : you need to set a CharacterModel on your Character component, and/or specify MovementRotatingModel and WeaponRotatingModel on your CharacterOrientation3D inspector. Check the documentation to learn more about this.");
			}
			...
			// Initialize intended rotation quaternions to current model rotation
			if (MovementRotatingModel != null)
			{
				_newMovementQuaternion = MovementRotatingModel.transform.rotation;
			}
			if (WeaponRotatingModel != null)
			{
		...
		public override void ProcessAbility()
		{
			...
			// Always get current direction for movement direction calculations, even if rotation is disabled
			...
			{
				// Still need to update ModelDirection and movement direction values even when rotation is disabled
				RotateModel();
			}
		}
		...
		protected virtual void RotateToFaceMovementDirection()
		{
			...
			// Update current direction (may already be set in ProcessAbility, but ensures it's current)
			_currentDirection = ForcedRotation || _controllerIsNull ? ForcedRotationDirection : _controller.CurrentDirection;
			...
			if (MovementRotationSpeed == RotationSpeeds.Smooth)
			{
				if (_currentDirection != Vector3.zero)
				{
					...
					// Use intended rotation for smooth interpolation (works even when visual rotation is disabled)
					Quaternion currentRotation = ApplyVisualRotation ? MovementRotatingModel.transform.rotation : _newMovementQuaternion;
					_newMovementQuaternion = Quaternion.Slerp(currentRotation, _tmpRotation, Time.deltaTime * RotateToFaceMovementDirectionSpeed);
				}
			}
		...
		protected virtual void RotateToFaceWeaponDirection()
		{
			...
			if (WeaponRotationSpeed == RotationSpeeds.Smooth)
			{
				if (_rotationDirection != Vector3.zero)
				{
					...
					// Use intended rotation for smooth interpolation (works even when visual rotation is disabled)
					Quaternion currentRotation = ApplyVisualRotation ? WeaponRotatingModel.transform.rotation : _newWeaponQuaternion;
					_newWeaponQuaternion = Quaternion.Slerp(currentRotation, _tmpRotation, Time.deltaTime * RotateToFaceWeaponDirectionSpeed);
				}
			}
			...
			if (WeaponRotationSpeed == RotationSpeeds.SmoothAbsolute)
			{
				...
				{
					_lastAim = _rotationDirection;
				}
				if (_lastAim != Vector3.zero)
				{
					_tmpRotation = Quaternion.LookRotation(_lastAim);
					// Use intended rotation for smooth interpolation (works even when visual rotation is disabled)
					Quaternion currentRotation = ApplyVisualRotation ? WeaponRotatingModel.transform.rotation : _newWeaponQuaternion;
					_newWeaponQuaternion = Quaternion.Slerp(currentRotation, _tmpRotation, Time.deltaTime * RotateToFaceWeaponDirectionSpeed);
				}
		...
		{
			// Only apply visual rotation if enabled, but always track the intended direction
			...
			// Always update ModelDirection and ModelAngles based on intended rotation (for animator)
			// Use the intended rotation quaternion instead of the actual transform rotation
			if (_shouldRotateTowardsWeapon && (_weaponRotationDirection != Vector3.zero))
			{
				ModelDirection = _newWeaponQuaternion * Vector3.forward;
			}
			else
			{
				ModelDirection = _newMovementQuaternion * Vector3.forward;
			}
			ModelAngles = _newMovementQuaternion.eulerAngles;
			
			// Use the normalized relative speed values that are already calculated
			// These are computed in ComputeRelativeSpeeds() and represent direction in model's local space
			// Round to whole numbers (-1, 0, or 1) including negatives
		...
		protected virtual void ComputeRelativeSpeeds()
		{
			...
			// relative speed - use intended rotation when visual rotation is disabled
			if ((_characterHandleWeapon == null) || (_characterHandleWeapon.CurrentWeapon == null))
			{
				...
				{
					_relativeSpeed = Quaternion.Inverse(_newMovementQuaternion) * _newSpeed;
				}
			}
			else
			{
				...
				{
					_relativeSpeed = Quaternion.Inverse(_newWeaponQuaternion) * _newSpeed;
				}
			}
		...
		protected override void InitializeAnimatorParameters()
		{
			...
			RegisterAnimatorParameter(_movementDirectionXAnimationParameterName, AnimatorControllerParameterType.Float, out _movementDirectionXAnimationParameter);
			RegisterAnimatorParameter(_movementDirectionZAnimationParameterName, AnimatorControllerParameterType.Float, out _movementDirectionZAnimationParameter);
		}
		...
		public override void UpdateAnimator()
		{
			...
			MMAnimatorExtensions.UpdateAnimatorFloat(_animator, _movementDirectionXAnimationParameter, _movementDirectionX, _character._animatorParameters, _character.RunAnimatorSanityChecks);
			MMAnimatorExtensions.UpdateAnimatorFloat(_animator, _movementDirectionZAnimationParameter, _movementDirectionZ, _character._animatorParameters, _character.RunAnimatorSanityChecks);
		}
	}
}